  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const res = await fetch(getApiUrl("/api/products?category=earrings"))
        if (!res.ok) {
        ...
        const mapped: Product[] = data.map((item) => {
          ...
          return {
            ...
            title: item.title || item.name || "Untitled Earrings",
            price: formattedPrice,
            image: item.image || "/earring1.jpeg",
          }
        })
    ...
    <div className="bg-white min-h-screen text-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            <div className="col-span-full py-12 text-center text-blue-600">Loading earrings…</div>
          ) : error ? (
            <div className="col-span-full py-12 text-center text-red-600">{error}</div>
          ) : products.length === 0 ? (
            <div className="col-span-full py-12 text-center text-slate-600">No earrings found.</div>
          ) : (
            products.map((item) => (
              <Card
                key={item.slug}
